...
public class PubSelectedImgsAdapter extends BaseAdapter {
    ...
    public View getView(int position, View view, ViewGroup arg2) {
        ...
        if (module.equals("show")) {
            ...
            Glide.with(context).load(mAllPath.get(position)).format(DecodeFormat.PREFER_ARGB_8888).into(holder.imageView);
        } else {
            if (isShowAddItem(position)) {
                Glide.with(context).load(R.mipmap.image_add).format(DecodeFormat.PREFER_ARGB_8888).into(holder.imageView);
                holder.delete_img.setVisibility(View.GONE);
            } else {
                ...
                if (mAllPath.get(position).contains("http")) {
                    ...
                    Glide.with(context).load("file://" + mAllPath.get(position)).format(DecodeFormat.PREFER_ARGB_8888).into(holder.imageView);
